...
export class PackagesPage extends BasePage {
  constructor(page) {
    super(page);
    this.packagesLink = page.getByRole("link", {
      name: "Packages",
      exact: true,
    });
    this.viewDetailBtn = page.getByRole("link", { name: "View Detail" });
    ...
    this.packageLocation =
      "//div[contains(@class,'MuiCardContent-root')]/child::div[3]";
    this.packagePrice =
      "//div[contains(@class,'MuiCardContent-root')]/child::div[4]";
    this.wishlistAdded = page.getByText("Package added to your wishlist");
  ...
  async getCardValues(targetName) {
    ...
    for (const packageCard of cards) {
      ...
      if (title.includes(targetName)) {
        ...
        const location = await packageCard
          .locator(this.packageLocation)
          .innerText();
  ...
  async selectPackageToWishlist(targetPackage) {
    ...
    await expect(this.wishlistAdded).toHaveText(
